...
- **第八课摘要**：注意力机制与Tranformer框架
  ...
  - **注意力**的操作过程（以翻译某个短句为例）：
    ...
    4. 在每一个时间步中：
      i. 使用当前**解码器状态** $$s_{t-1}$$ 和所有**编码器隐藏状态** $$h_i$$，计算对齐分数 (Alignment Scores) $$e_{t,i}$$
      ii. 对 $$e_{t,i}$$ 使用Softmax归一化，得到**注意力权重** (Attention Weights) $$α_{t,i}$$
      iii. 根据 $$α_{t,i}$$ 对所有 $$h_i$$ 进行**加权求和**，得到**上下文向量**(Context Vector) $$c_t$$
      iiii. 解码器使用 $$s_{t-1}$$，$$y_{t-1}$$ 和 $$c_t$$ 来计算下一个状态 $$s_t$$ 和预计输出 $$y_t$$
