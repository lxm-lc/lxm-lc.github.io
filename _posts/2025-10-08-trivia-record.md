...
- 晚上发现志愿者活动的素拓全被驳回了，因为没有本人真名显示；想想这确实是合理的，但是哈基第二课堂把我的志愿活动全归于文体活动是什么鬼啊，我不理解，要不是因为怕这个影响判定，我也不会只放录取界面😭哈基电院 ~~哦不对，电院已经似了~~ 什么时候能第二课堂加一个“最后参与成功”的选项啊，这样之后全部系统指定，也没那么多奇奇怪怪的事情了🥲
- 晚上和工试群u**散步**(●'◡'●) **大家都好可爱，喜欢大家喵~~**
- 我说散步是对的😭给我无聊的生活带来了无限乐趣
...
### `20250919`
- ？怎么不在“志愿活动”下的志愿活动不能算在素拓里啊`:anon_angry:`哈基电你这家伙🥲为什么去年（综测条例里这一项未修改）就能算是啊……我不懂啊，不要搞我啊呜呜呜，孩子当了那么久苦力就是想拿一点素拓，我也不容易啊😭（但是确实要好好看综测条例，不是吗，要是真搞不定我也认栽了`:tieba_hehe:`后编：这个确实没办法了，呜呜呜；后续：原来和组织单位有关，那没事了ww
...
- 然后开始糊CV报告，感觉我来来回回花在这个上面的时间不下10h啊`:tieba_laugh:`这就是水报告的含金量嘛😭下次还是不要用这么多时间为好🥲
...
- 然后我又浏览了一遍[这位博主的主页](https://www.youtube.com/@SirajRaval/streams)，感觉比较多的都是零散的教程，例如在短时间内实现并讲解某小项目，单看起来还是很有意思的，但是不适合作为系统学习的资料；但是我作为一个初学者，在一开始就掌握所有内容是不现实的，我或许可以通过看这些小视频来**激发兴趣**，并且大多都当作**科普了解**，这都算是**相当好的学习体验了**💕
...
- 凹糟的多媒体课，需要电脑但在中院，游戏本没有电源卡的要死，**哈基排课**你这家伙🙌😭🙌
...
- 晚上继续看了一个与分辨红酒品类好坏相关的project，但是鉴于时间问题，只看了数据统计的部分，真的感觉python自带的plt太神了🤯怎么能画出复合图🤣机器学习的部分看起来不多，我打算明天物理课下再看，感觉挺有意思的
- 以及联想到之前西瓜书里有说要**针对每个问题，选择适合它们的算法**；或许，用**数据统计**的方式可以**整体感知数据状态**，从而更方便我们判断机器学习用的方法等吧💕
- 欸其实感觉保持现在这个状态刷项目（就是看看具体数据统计、机器学习代码实现等而并不直接攥写代码），我现在的库存应该会比较快就清空了欸；感觉到最后其实就是基本了解各个算法的代码实现思路，也不一定需要完全会写（AI会出手是这样的，我只要能看懂、有基本debug方向就基本能满足目前的需求）；此后就可以CS231n启动啦~还是很期待结合了project的、我之前未接触过的专业课程的💕~~ ( •̀ ω •́ )y
...
- 早十没课，回到寝室继续看昨天跑的代码，才到42/100😅怎么能慢成这个鬼样子，而且跑出来效果也……不好评价，某图车旁边沾的广告被识别为车牌了，家人们谁懂这种无助感啊😭
...
### `20250924`
- 上午听了概统，感觉舒适，然后续了哈喽单车的卡，发现新卡没充进去是因为换了电话号码，这下我成纯小丑了🤡 （？才发现🤡居然是`:clown_face:`而不是`:clown:`嘛？哈基小丑你这家伙！`:anon_angry:`）
...
- 补了一些物理课，感觉对电势那一堆知识++（我看是高中物理全忘光了吧`:tieba_hehe:`），作业或许可以做了，但是哥们真有时间做吗`:tieba_happy:`（我还挺想好好做然后校对答案的，这学期物理一定要拿高分啊😭）
...
> **有用的论文阅读prompt存档**：
> 1. 你是一个AI领域专家。现在，请你以专业的视角阅读本论文：1. 对论文主要内容进行摘要、概述；2. 按文章内部顺序对本文提及的内容按顺序向我梳理；3. 向我简要介绍文章背景中的重要基础知识；4. 向我呈现并说明文章中的重要基本概念；5. 说说本文成果的具体应用价值
> 2. 请你为我翻译，并对关键概念用简单的语言解读；具体格式为表格，最左边一栏是原文，中间一栏是译文，右侧是本小段译文中包含的关键概念
- 为什么说**高耗时、高内存是部署的主要障碍**：因为模型往往参数量巨大、结构复杂；在内存上，模型推理需要大量显存，而个人的手机、电脑上显存往往不足，这就使得本地设备无法运行它们，只能上云，模型普惠性降低，且一台服务器能服务的用户就少；在耗时上，模型从输入到输出耗时太久，从而容易使用户流失，同时也无法完成一些实时性强的活动（如VR/AR），且计算成本高
- 为什么**DiT容易受极端值（异常值）影响**：因为其计算耗能大等问题，我们一般需要压缩它，从较长的浮点数压缩为较小的整数，这种压缩一般参考原数范围 $$[V_{min}, V_{max}]$$，然后再映射到特定空间，所以如果原来的数值都很集中，那就非常皆大欢喜；如果出现了异常值，我们保留它，正常的数就会被压的很紧，精度下降，如果舍去它，正常的数是好了，但是说不准这个极端值有特殊意义价值，扔了它对模型性能影响很大——所以我们要在“舍”和“留”之间寻求平衡
- **CLQ的创新点之一——正交量化**：简而言之，就是找到一组正交基，让其中的一个或几个表示极端值的信息，其他的表示常规值的信息（此时，量化范围很小，精度损失可控）；这样，不处理极端值直接压缩造成的常规值精度损失，以及直接裁剪极端值导致的潜在重要信息遗失这两个现象都得到了比较好的规避；可以理解为把一个难以兼顾的大问题拆成了两个子问题
- **U-Net与DiT**：U-Net是一种卷积神经网络，在最早的扩散模型中扮演了**去噪器**（Denoiser）的角色，预测图片中被添加的噪声，并从图中将其抹去；但是，由于卷积神经网络的特性，U-Net擅长局部观察，而DiT用善于全局思考的Transformer逐渐取代了它，从而解锁视觉生成模型的更大潜力
- **CLQ框架中的三个重要技术点**：
  - **CBC：跨块校准 (Cross-Block Calibration)**：要求在校准当前块（Block）中的任何一层时，该块之前的所有块都必须使用最终的量化权重进行推理；解决了**量化误差累积问题**
  - **OBS：基于正交的平滑 (Orthogonal-based Smoothing)**：它通过正交变换这种数学工具，实现了信息等价的重新分配，将高集中的尖峰分布变成了均匀平滑的温和分布（如：把极端值分散到各个正常值中去），从而使得原本困难的量化任务变得容易且高效
  - **CLPS：跨层参数搜索 (Cross-Layer Parameter Searching)**：引入一个前瞻性的优化目标（对全局有利），而**不是局限于局部最优**
- 今晚又看了0.5篇文章，感觉这一篇还是涉及了很多新概念的，并且有不少的数学推演，所以看起来有点慢，但是内容很有意思；美中不足的是我看文章的过程被反复打断`:anon_angry:`这就非常败兴，否则**感觉看文章还是超级超级享受的呢**🥰👩‍🎓📚🏫

##### 科研任务：阅读论文*3 (1.5/3)


### `20251006`
- 今天上午一起床就把所有衣服洗了，中午开始跑一个黄诗扶的声音模型（只能说这个声音太ngaam粤语了，可惜她并没有多少粤语歌唱过，而且粤语细听还是不太准的），在跑模型的同时继续快乐读论文喵😸
- 今天穿了女装楼同款xqz嘻嘻嘻🥰
- **矩阵的局部排序与分块计算**：我们需要对原矩阵进行排列，把其中更像极端值的放到右50%，再用Hadamard矩阵对其进行变换，从而把极端值平均到每个通道里去，让每个通道里的数据更平滑（不需要严格从极端到平滑分类，这是因为在初步分类之后，Hadamard矩阵的变换效果就非常好了），并且通过乘一个系数（真正的标准正交Hadamard矩阵 $$H_{std}$$ 必须包含一个缩放因子 $$\frac{1}{\sqrt{N}}$$ ），让变化后的矩阵的 $$L_2$$ 范数平方（也就是总信息能量）在平滑前后不变
- **为什么把矩阵分块运算效率高**：GPU（图形处理器）拥有数千个核心，最擅长的就是**同时执行大量独立的简单任务**；单个矩阵的矩阵乘法复杂度是 $$O(n^3)$$，计算开销大，而如果将其拆解成小矩阵进行计算，就容易并行处理，较为快速的得到结果，可以说是大大提升了**效率**
- 在Transformer架构中，一个标准的Transformer Block主要分为两部分：自注意力子层（Attention Sublayer，负责全局信息交互和捕捉输入序列中元素间的关系，较**广**）和前馈网络子层（FFN Sublayer，负责局部特征增强和非线性变换，较**深**），其中，FFN敏感性更高一些，所以需要更强的平滑，从而保持精度
- 为什么要**施加扰动**？施加扰动是在上游的某个位置（ $$L_O$$ ）仿照实际误差形式加一个小的扰动，然后观察其会放大到下游的哪一层（ $$L_T$$ ），从而判断网络的**敏感点**，并对其进行控制；相当于说，如果我们能让 $$L_T$$ 的输出保持稳定，就相当于控制住了 $$L_O$$ 误差最主要的传播方向
- 跑了一整天的**声音克隆**模型，好累好费时间啊，而且让我在做感兴趣的事情的时候分心，下次不再跑模型了，还是专心做事喵
- 虽然我觉得晚上肯定还是补课比较好，但是我感觉我十分需要继续学习大模型的知识，但是感觉心难以静下来做事，估计等会儿又要看模型，又要看手机，啊啊啊啊啊啊啊啊啊好难受，我不喜欢自己的注意力分散的感觉，所以我觉得我应该寻求一种方式，使得我的注意力上升
- 同时，我感觉我最近有些沉迷手机，这让我做事被反复打断，这是不对的
- 并且，我感觉最近的效率有些低，各种plan也没有很好的被执行，这不是一件好事
- 为了解决这一现状，我想，我需要合理散聚时间，也就是说，该玩的时候玩，该学的时候学，不要把自己扣牢在埋头苦干上，也应该少做自己不感兴趣的事情，或是说，把兴趣牵引向学习多点，而手机上的QQ和网站中的水源应该非常少去看它们！
- 为了达成这一目的，我想，我应该即刻行动起来！
- 比如立刻去心无旁骛的跑步，回来之后立刻洗澡，洗完澡就应该坐到电脑前，继续看cs231n，并且要注意连贯性——即使我有内容不会而去搜索，也不要过于分心，且要重视整体效率！
- 在文字输出上，我应该focus到本文，享受文本输出而不是寻求文本交互！
- 在此之外，我应该培养习惯，让自己能够早睡早起，多做点事，劳逸结合！
- 我现在应该做的第一件事是换衣服然后去跑步！

##### 科研任务：阅读论文*3 (2/3)

### `20251007`
- 上午写完了上周的周记，感慨啥也没做的国庆就要过去了😿或许是在实务上没有做很多，但是进程上推进了（好比是量变不多，但是发生质变！）下一周恢复正轨，还是继续进行量变吧，争取多做点
- 才发现原来我昨日`git push`不成功是因为端口没设置7897hhhh（跑声音克隆模型会占这个端口，所以我把我端口设到了1000先了，今早已经换回）
- 刚发现网页端渲染不出latex公式……我的母语是无语……😶看看能不能修吧
- 早上起来看cs231n先，aaaa感觉假期已经很久没有看过它了qwq🫠
- **第七课摘要**：RNN（循环神经网络）
  - 模型训练公式为 $$h_t=f_W(h_{t-1},x_t)$$ ，本质上是用**递归**手段，结合原有的模型以及新的输入，来训练新的模型
  - 输出则依靠公式 $$y_t=f_{W_{hy}}(h_t)$$ 生成结果
  - RNN有一个非常明显的好处，就是不限制上下文长度（这是因为RNN有权重共享机制，无论序列多长，它都使用同一组参数进行循环计算，因而适合长度可变的输入）；但是随着上下文变长，可能会出现梯度消失等情况，也就导致模型对很久前接触的信息可能已遗忘；同时，RNN必须严格按顺序计算（计算 $$h_T$$ 之前，必须先完成 $$h_{T-1}$$ 的计算），这使得其耗时长，且难以进行并行化处理
  - 相比之下，下一节课所要提及的Transformer模型就会限制上下文长度，且随着数据量的增加，Transformer的计算量呈 $$O(n^2)$$ 形式的增长，所以后期我们需要尝试新方法去克服这些问题，让模型达到更好的效能
- **第八课摘要**：注意力机制与Tranformer框架
  - CNN只能由一个输入数据得到一个新的输出数据，而RNN的出现可以让我们输入序列化数据，实现一对多/多对一/多对多的输入输出新模式
  - **注意力**的操作过程（以翻译某个短句为例）：
    1. 输入序列 $$x_1, x_2, ... , x_T$$ 首先进入嵌入层 (Embedding Layer)，转化为**词向量** (Word Vectors)，然后送入编码器 (Encoder)
    2. 编码器接收词向量后，生成**隐藏状态** (Hidden States) $$h_1, h_2, ... , h_T$$，这些 $$h_i$$ **不是简单的量化**，而是**捕获**了该词及其**周围词汇信息**的上下文向量（矩阵/张量）——这是“**回看**”的全部来源
    3. 解码器 (Decoder) 接收初始状态 $$s_0$$，并进入**迭代生成**（Processing Steps）阶段;在每个时间步 $$t$$，解码器准备生成输出词 $$y_t$$ 
    4. 在每一个时间步中：
      - 1. 使用当前**解码器状态** $$s_{t-1}$$ 和所有**编码器隐藏状态** $$h_i$$，计算对齐分数 (Alignment Scores) $$e_{t,i}$$
      - 2. 对 $$e_{t,i}$$ 使用Softmax归一化，得到**注意力权重** (Attention Weights) $$α_{t,i}$$
      - 3. 根据 $$α_{t,i}$$ 对所有 $$h_i$$ 进行**加权求和**，得到**上下文向量**(Context Vector) $$c_t$$
      - 4. 解码器使用 $$s_{t-1}$$，$$y_{t-1}$$ 和 $$c_t$$ 来计算下一个状态 $$s_t$$ 和预计输出 $$y_t$$
        - 其中，$$s_{t-1}$$ 和 $$y_{t-1}$$ 代表了*内部历史*，告诉模型“我从哪里**来**”，从而保证句子连贯、符合语法；而 $$α_{t,i}$$ 是*外部对齐*，告诉模型“**输入中**哪个词最重要”，确保生成的句子忠实于源文本的含义
    - **注意力权重矩阵的可视化**
    ...
    - 请注意，这里的查询向量 $$Q$$ 和 键向量 $$K$$（和上文编码器的输出相对应），我们可以用**点积**的方式判定其**相似度**，从而计算其对应的注意力分数 $$\text{Score}=Q·K^T$$，但是鉴于Softmax的计算公式是 $$\text{Softmax}(x_i) = \dfrac{e^{x_i}}{\sum_{j} e^{x_j}}$$，如果有一项 $$\text{Score}$$ 特别大，就可能出现在 $$\text{Softmax(x_i)}$$ 中这一项趋于1，其他项趋于0的情况，从而更可能导致**梯度消失**；在点积后除以 $$\sqrt{d_k}$$（也即 $$K$$ 的维度）可以缓解这一情况
​- 我也是服了，修这个 $$\LaTeX$$ 语法显示花了我好久hhh 而且我发现连着梯子的话就没法渲染出公式，也没法渲染出边目录，也是神人了
- 截止`17:00`，从第七课的倒数30min，看完了第八课的前40min，感觉看的内容不是很多呢😿但是基本上看的都能明白意思（AI辅助学习是这样）😼其他的就明天再看好啦
- 晚上📦到了楼u🥰打水回寝的时候被路人搭讪要微信了😰不过我脑子转的飞快，回绝了先，感觉对方人还挺礼貌的嘞🐖（怎么这个`:pig2:`的渲染这么丑，我要水源那种可爱的渲染方式呜呜😿）
- 晚上学习了**SSH**的基本用法（实则只学会了`ssh name@example_host`的语法🤣之后要用什么再学吧），并且写完了上次的大物实验报告，把新一次的大物实验预习报告写了60%左右
##### 看课进度：cs231n-class (7.5/18) cs231n-project (0/3)


### `20251008`
- 上午做了大扫除，打理了我的桌面（物理意义上），也把地板上的碎发通通扫光，继续学cs231n了
- **第八课摘要**：注意力机制与Tranformer框架（续·此处写的是脱离了RNN框架的注意力机制）
  - 先把**数据向量** (data vector) 拆分成键 (key) 和值 (value)，并将其与输入的查询向量 (query vector) 结合，计算注意力，再对每一列 (也即每一个单独的query) 进行 $$\text{Softmax}$$ 归一化，最后得出总的注意力序列——**交叉注意力层** (cross-attention layer): 在两个不同的集合之间进行注意力交叉
  - 与上面说的**交叉注意力** (cross-attention) 平行的，还有**自注意力** (self-attention)，也就是把**输入向量**投射到三个部分（查询向量、键和值），其余步骤和上述相同
  - 在实践上，硬件上少量个大矩阵的乘法效率要高过多个小矩阵的乘法，所以我们会选择拼接 $$W_Q, W_K, W_V$$ 为一个大矩阵，并将其与 $$X$$ 相乘，得到式子 $$[Q K V]=X[W_Q W_K W_V]$$
  - **置换等变性** (permutation equivariance): 改变输入顺序，输出数据相同，只是顺序改变（自注意力实际上*不关注*输入顺序，本质上是基于无序的向量集合而非有序的向量序列进行操作）；如果告诉神经网络输入的顺序是必要的，我们可以使用**位置嵌入** (positional embedding)，插入数据以告知其各个向量的顺序
  - **掩码自注意力** (masked self-attention) 可以被用于我们希望某query只能查询部分输入的情况，此时我们只需要把原本计算出来的注意力改为 $$-\infty$$，这样，经过 $$\text{Softmax}$$ 机制之后，它们的权重就会降到 $$0$$，也就意味着此处的输出 $$y$$ 将不会依赖于该索引处计算的值向量（从而**控制哪些输入可相互作用**）
  - **多头自注意力** (multi-headed self-attention) 是**同时**进行**多次独立**的自注意力计算，并将这些结果合并起来
- **transformer block 流程总结**：我们把一个 Transformer Block 看作一个复杂的**信息处理工厂**，它的工作是接收半成品材料（输入序列的表示），然后进行两次关键的加工和质量控制，最终产出更精炼的成品
  - 阶段一：自注意力 (Self-Attention): 让信息元素（词语）之间互相交流，理解彼此的关系；在序列维度上进行**信息融合和关系建立**

| Transformer 组件 | 作用 | 类比 |
| :--- | :--- | :--- |
| **自注意力** (Self-Attention) | 建立关系：让序列中的每个词都去“询问”其他所有词，并根据重要性聚合出一个新的、包含全局上下文的表示。 | 🏭 信息交流会：所有零件（词语）坐在一起开会。每个零件都发言（Query），听取其他所有零件的介绍（Key），然后根据发言内容的重要性，从所有零件的库存（Value）中，按比例提取所需信息，形成一个共识（上下文向量）。 |
| **残差连接** (Residual Connection) | 确保历史：将自注意力车间的输出与原始输入相加。 | ➕ 直通管道：在加工过程中，将原始的半成品（输入）用一条直通管道连接到车间的出口，然后将“加工后的新材料”和“原始材料”混合，确保原始材料的信息永不丢失。 |
| **层归一化** (Layer Normalization) | 质量控制 I：将相加后的结果标准化。 | 📊 统一标准尺：对混合后的材料进行质量检查和标准化，确保每批材料的颜色、密度等属性都在可控范围内，为下一阶段的加工做好准备。 |

  - 阶段二：前馈网络 (FFN / MLP): 进入 FFN 阶段进行非线性精加工；在特征维度上进行**非线性增强和信息提炼**
